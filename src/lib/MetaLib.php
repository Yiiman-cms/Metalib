<?php
	...
	namespace YiiMan\YiiLibMeta\lib;
	
	
	use YiiMan\YiiLibMeta\module\models\Metadata;
	...
	class MetaLib {
		...
		public function set( $meta_key , $value , $module_id = 0 , $update = false ) {
			if ( $update ) {
				...
				if ( is_array( $value ) || is_object( $value ) ) {
					...
					$save = $meta->save();
					if ( ! $save ) {
                        $this->set( 'metaError' , $meta->errors , 0 );
					}
		 ...
		 * @param $module_id
		 *
		 * @return \YiiMan\YiiLibMeta\module\models\Metadata|null
		 */
